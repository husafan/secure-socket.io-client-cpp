#include "ssio_socket.h"
#include "internal/ssio_packet.h"
#include "internal/ssio_client_impl.h"
...
#undef SYNTHESIS_SETTER
    ...
    void socket::impl::on_connected()
    {
        ...
        if(!m_connected)
        {
            ...
            while (true) {
				...
				ssio::packet front_pack = std::move(m_packet_queue.front());
                m_packet_queue.pop();
				m_packet_mutex.unlock();
    ...
    {
        NULL_GUARD(m_client);
        ssio::client_impl *client = m_client;
        m_client = NULL;
    ...
    void socket::impl::send_packet(ssio::packet &p)
    {
        NULL_GUARD(m_client);
        if(m_connected)
        {
